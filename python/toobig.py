...
def LucasKanadeAffine(It, It1, rect):
   ...
   # create grid of points on image
    ...
    # use full template grid or rect grid?
    ...
    while np.linalg.norm(delta_p) >= threshold and iter < maxIters:
        ...
        warped_img = cv2.warpAffine(It1, M, (It1.shape[1], It1.shape[0]))

        # create grid of translated points for the warped image
        # using same grid as before on top of the warped image
        warped_spline = RectBivariateSpline(x, y, warped_img.T)   # try redifining x and y
        I = warped_spline.ev(xt, yt).flatten()                    # use .ev() to get grid in warped image

        # get image gradient using .ev() and unroll matrices
        I_x = warped_spline.ev(xt, yt, dx=1).flatten()             # x derivative of warped image
        I_y = warped_spline.ev(xt, yt, dy=1).flatten()             # y derivative
