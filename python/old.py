...
def LucasKanade(It, It1, rect):
    ...
    warp = np.array([[1, 0, p[0]],
                     [0, 1, p[1]]])
    img_warped = cv2.warpAffine(It1, warp, (1000, 1100))
    ...
    I = np.ravel(img_warped[int(y1) : int(y2) + 1, int(x1) : int(x2) + 1])      # need to subtract with below, so should be same shape
    T = np.ravel(It[int(y1) : int(y2) + 1, int(x1) : int(x2) + 1])              # should just be rectangular portion of It image
    print(I.shape)

    # image gradient
    # I_x = spline.ev(x, y, dx=1)
    # I_y = spline.ev(x, y, dy=1)
